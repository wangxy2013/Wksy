...
public class HttpRequest1 implements Runnable
{
    ...
    private Map<String, List> valuePair;
    private IRequestListener mIRequestListener = null;
    private JsonHandler      mHandler          = null;
    private File    mFile;
    private Context mContext;

    public HttpRequest1(int action, String type, String url, IRequestListener listener, JsonHandler handler)
    {
        mAction = action;
        urlRequest = url;
        mType = type;
    ...
    public HttpRequest1(Context mContext, int action, String type, String url, Map<String, List> valuePairs,
    ...
    public HttpRequest1(int action, String type, String url, File mFile,
                        IRequestListener listener, JsonHandler handler)
    {
        this(action, type, url, listener, handler);
        this.mFile = mFile;
    ...
    private String doPost() throws Exception
    {
        ...
        // String data = ConvertMap2Json.buildJsonBody(valuePair, 0, false);

        String data = JSON.toJSONString(valuePair);
        LogUtil.e("TAG", "postData>" + data);
        ...
        if (!StringUtils.stringIsEmpty(ConfigManager.instance().getToken()))
        {
            requestParams.put("token", ConfigManager.instance().getToken());
        }
